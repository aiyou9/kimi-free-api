...
var util = {
  ...
  generateCookie() {
    ...
    const items = [
      `Hm_lvt_4532beacc312859e0aa3e4a80566b706=${timestamp - Math.round(Math.random() * 2592e3)}`,
      ...
      `_ga_31QPQG2YYD=GS1.1.${timestamp - Math.round(Math.random() * 2592e3)}.17.0.${timestamp - Math.round(Math.random() * 2592e3)}.0.0.0`,
      `Hm_lpvt_4532beacc312859e0aa3e4a80566b706=${timestamp - Math.round(Math.random() * 2592e3)}`,
      `Hm_lpvt_358cae4815e85d48f7e8ab7f3680a74b=${timestamp - Math.round(Math.random() * 2592e3)}`,
      `_ga_YXD8W70SZP=GS1.1.${timestamp - Math.round(Math.random() * 2592e3)}.35.1.${timestamp - Math.round(Math.random() * 2592e3)}.0.0.0`
    ];
    return items.join("; ");
  },
...
var LogWriter = class {
  #buffers = [];
  constructor() {
    import_fs_extra5.default.ensureDirSync(config_default.system.logDirPath);
    this.work();
  }
  push(content) {
  ...
  writeSync(buffer) {
    import_fs_extra5.default.appendFileSync(import_path5.default.join(config_default.system.logDirPath, `/${util_default.getDateString()}.log`), buffer);
  }
  async write(buffer) {
    await import_fs_extra5.default.appendFile(import_path5.default.join(config_default.system.logDirPath, `/${util_default.getDateString()}.log`), buffer);
  }
  flush() {
    if (!this.#buffers.length)
      return;
    import_fs_extra5.default.appendFileSync(import_path5.default.join(config_default.system.logDirPath, `/${util_default.getDateString()}.log`), Buffer.concat(this.#buffers));
  }
  work() {
    if (!this.#buffers.length)
...
async function createCompletion(model = MODEL_NAME, messages, refreshToken, useSearch = true, retryCount = 0) {
  return (async () => {
    ...
    const convId = await createConversation(`cmpl-${util_default.uuid(false)}`, refreshToken);
    const token = await acquireToken(refreshToken);
...
async function createCompletionStream(model = MODEL_NAME, messages, refreshToken, useSearch = true, retryCount = 0) {
  return (async () => {
    logger_default.info(messages);
    const refFileUrls = extractRefFileUrls(messages);
    const refs = refFileUrls.length ? await Promise.all(refFileUrls.map((fileUrl) => uploadFile(fileUrl, refreshToken))) : [];
...
function messagesPrepare(messages) {
  const content = messages.reduce((content2, message) => {
    ...
    return content2 += `${message.role || "user"}:${wrapUrlsToTags(message.content)}
`;
  }, "");
  return [
    { role: "user", content }
...
async function uploadFile(fileUrl, refreshToken) {
  ...
  result = await import_axios.default.post("https://kimi.moonshot.cn/api/file/parse_process", {
    ids: [fileId],
    timeout: 12e4
  }, {
    headers: {
...
var chat_default2 = {
  ...
  post: {
    "/completions": async (request) => {
      request.validate("body.messages", import_lodash13.default.isArray).validate("headers.authorization", import_lodash13.default.isString);
      ...
      if (request.body.stream) {
        const stream = await chat_default.createCompletionStream(model, messages, token, request.body.use_search);
        return new Response(stream, {
          type: "text/event-stream"
        });
      } else
        return await chat_default.createCompletion(model, messages, token, request.body.use_search);
    }
  }
};
